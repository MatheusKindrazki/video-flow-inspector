...
describe("loadConfig adaptive pipeline settings", () => {
  it("parses Gemini escalation, output, timeout, and frame selection options", () => {
    ...
    process.env.GEMINI_ESCALATION_MODEL = "gemini-3.5-flash";
    ...
    expect(config.providers.gemini?.model).toBe("gemini-3.1-flash-lite");
    expect(config.gemini).toMatchObject({ escalationEnabled: true, escalationModel: "gemini-3.5-flash", escalationConfidenceThreshold: 0.7, escalationOnCritical: false, maxOutputTokens: 4096, timeoutReduceFrames: false });
