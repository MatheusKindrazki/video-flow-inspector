...
describe("GeminiProvider", () => {
  it("does not retry permanent 4xx failures", async () => {
    ...
    const provider = new GeminiProvider("unused", "gemini-3.1-flash-lite", 3, 100, { client, retryDelayMs: 0 });
  ...
  it("retries transient 5xx failures", async () => {
    ...
    const result = await new GeminiProvider("unused", "gemini-3.1-flash-lite", 2, 100, { client, retryDelayMs: 0 }).analyze(context);
  ...
  it("does not mark valid pretty-printed JSON as repaired", async () => {
    ...
    const result = await new GeminiProvider("unused", "gemini-3.1-flash-lite", 1, 100, { client, retryDelayMs: 0 }).analyze(context);
  ...
  it("escalates once on low confidence when enabled", async () => {
    ...
    const result = await new GeminiProvider("unused", "gemini-3.1-flash-lite", 1, 100, { client, escalationEnabled: true, escalationConfidenceThreshold: 0.5, retryDelayMs: 0 }).analyze(context);
    expect(client.models).toEqual(["gemini-3.1-flash-lite", "gemini-3.5-flash"]);
  ...
  it("preserves first-model usage and combines retry stats when escalating", async () => {
    ...
    const result = await new GeminiProvider("unused", "gemini-3.1-flash-lite", 2, 100, { client, escalationEnabled: true, escalationConfidenceThreshold: 0.5, retryDelayMs: 0 }).analyze(context);
  ...
  it("escalates when the Lite response remains ambiguous after repair", async () => {
    const client = clientWith(["not JSON", valid()]);
    const result = await new GeminiProvider("unused", "gemini-3.1-flash-lite", 1, 100, { client, escalationEnabled: true, retryDelayMs: 0 }).analyze(context);
    expect(client.models).toEqual(["gemini-3.1-flash-lite", "gemini-3.5-flash"]);
  ...
  it("does not escalate when disabled", async () => {
    ...
    await new GeminiProvider("unused", "gemini-3.1-flash-lite", 1, 100, { client, escalationEnabled: false, retryDelayMs: 0 }).analyze(context);
    expect(client.models).toEqual(["gemini-3.1-flash-lite"]);
  });
